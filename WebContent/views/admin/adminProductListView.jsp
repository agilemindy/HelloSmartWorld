...
<body>
	...
	<div class="outer">
		...
		<div align="right" style="margin-right: 150px; margin-bottom: 20px" >
			<button id="enrollButton" onclick="location.href = '/smartWorld/productEnrollForm.do'">제품등록</button>
		</div>
		<div align="right" style="margin-right: 150px; margin-bottom: 20px" >
			<h4> 총 <%= proCount %> 개 </h4>
		</div>
		...
		<div class="pagingArea" align="center">
			<!-- 맨 처음으로 (<<) -->
			<button onclick="location.href='<%=contextPath%>/adminProductListView.do?currentPage=1'"> &lt;&lt; </button>
		
			<!-- 이전페이지로(<) -->
			<%if(currentPage == 1){ %>
			<button disabled> &lt; </button>
			<%}else{ %>
			<button onclick="location.href='<%= contextPath %>/adminProductListView.do?currentPage=<%= currentPage-1 %>'"> &lt; </button>
			<%} %>
			...
			<%for(int p=startPage; p<=endPage; p++){ %>
				...
				<%}else{ %>
				<button onclick="location.href='<%=contextPath %>/adminProductListView.do?currentPage=<%= p %>'"> <%= p %> </button>
				<%} %>
			...
			<!-- 맨 끝으로 (>>) -->
			<button onclick="location.href='<%=contextPath%>/adminProductListView.do?currentPage=<%=maxPage%>'"> &gt;&gt; </button>
		</div> 
		<br><br>
