...
/**
 * Servlet implementation class SalesListSortAscServlet
 */
@WebServlet("/sortAsc.do")
public class SalesListSortAscServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SalesListSortAscServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//페이징 처리
				...
				ArrayList<Order> list = new AdminService().selectSalesListSortAsc(pi);
				
				request.setAttribute("list", list);
				request.setAttribute("pi", pi);
				
				request.getRequestDispatcher("views/admin/salesListView.jsp").forward(request, response);
	}
