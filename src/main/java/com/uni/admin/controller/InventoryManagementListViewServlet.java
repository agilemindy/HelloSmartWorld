...
/**
 * Servlet implementation class InventoryManagementListViewServlet
 */
@WebServlet("/inventoryManagementListView.do")
public class InventoryManagementListViewServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InventoryManagementListViewServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//총게시글 갯수
		invenCount = new AdminService().getInvenCount(); // 입출고 건수 알아오기
		...
		maxPage = (int)Math.ceil((double)invenCount/boardLimit);
		...
		PageInfo pi = new PageInfo(invenCount, currentPage, startPage, endPage, maxPage, pageLimit, boardLimit);
		
		ArrayList<Pro_Detail> list = new AdminService().selectInvenList(pi); // 입출고 내역 받아오기
		
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		
		request.getRequestDispatcher("views/admin/inventoryManagementListView.jsp").forward(request, response);
	}
